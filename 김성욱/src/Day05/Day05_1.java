...
public class Day05_1 { // c s
	...
	public static void main(String[] args) { // m s
		...
		// 제어문 : if[논리(t/f) 제어검사], switch[데이터 검사]
			// if : 경우의수가 정해져있지 않는 경우
				// true 이면 실행문처리후 모든 if 종료
			// switch : 경우의수가 정해져 있는 경우 [ 속도 빠름 ]
				 // case 검사후 아래 모든 case 처리 => break ; [ 반복문혹은 switch 탈출 ]
		...
		int 점수1 = 80 ; int 점수2 = 90 ; int 점수3 = 100;
			// 세 점수의 평균을 계산후 : 90점 이상이면 A등급 80점이상이면 B등급 70점이상이면 C등급 그외 탈락
		int 평균 = (점수1 + 점수2 + 점수3)/3;
		switch(평균/10) {
			
		case 10 : // 100점에는 break 안걸음 , 밑에도 실행시키려고
