...
public class Day05_2 { // c s
	...
	public static void main(String[] args) { // m s 
		...
		// 배열 : 특정 기준으로 나열
			...
			// 동일한 자료형의 여러 변수를 관리 => 서로 다른 자료형은 불가
			// 클래스를 이용한 배열 선언
		...
		System.out.println("배열의 0 인덱스 : " + 정수배열2[0]); // 인덱스 위치의 값
		System.out.println("배열의 0 인덱스 : " + 정수배열2[1]); // 인덱스 위치의 값
		System.out.println("배열의 0 인덱스 : " + 정수배열2[2]); // 인덱스 위치의 값
		// 배열의 길이
		System.out.println("배열 : " + 정수배열2.length); // 배열내 요소 개수 [ 배열의 길이 ]
		System.out.println("배열 : " + 정수배열2.toString()); // 배열의 주소값
		
			// 문제 1 : 반복문을 이용한 0~99까지 int형 배열에 각 인덱스에 저장하고 출력
			int[] 배열1 = new int[100]; // int형 100개 저장할 수 있는 배열
			for( int i=0; i<100; i++) {
		...
		// 2차원 배열 [행인덱스][열인엑스]
			...
			// 열 = 세로 = column
			int[][] 배열2 = new int[2][3];
			int[][] 배열3 = { {10,20,30}, {40,50,60} }; // 1행 2행
			배열2[0][0] = 80;
			배열2[0][1] = 90;
			배열2[0][2] = 100;
			배열2[1][0] = 40;
			...
			// 문제 2 : 0~49 까지 첫번째 행 저장 50~99 까지는 두번쨰 행 저장하는 2차원 배열 선언하고 출력
