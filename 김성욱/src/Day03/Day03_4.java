	...
	public static void main(String[] args) { // m s
		...
		while(true) {
			...
			while(true) { // 무한루프      w s
			 ...
			 if (콜라구매수 == 0 && 커피구매수 == 0 && 스무디구매수 == 0) { System.out.println("   *** 장바구니 비어있음 ***   ");}
			 else {
			 	System.out.println("\t\t\t\t제품명\t구매수량\t금액");
			 	System.out.println("\t\t\t\t콜라\t"+콜라구매수+"\t"+콜라구매수*500);
			 	System.out.println("\t\t\t\t커피\t"+커피구매수+"\t"+커피구매수*600);
			 	System.out.println("\t\t\t\t콜라\t"+스무디구매수+"\t"+스무디구매수*700);
			 ...
			 if( 선택 == 1) { // 콜라 처음에 11개 이상 주문하는거 막기  코드 짜보기
				 if( 콜라재고== 0 ) { System.out.println("[준비중] 재고가 없습니다.");}
				 else { System.out.println("콜라를 담았습니다"); 콜라구매수++; }
			 }
			 if( 선택 == 2) {
