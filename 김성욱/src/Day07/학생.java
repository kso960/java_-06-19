...
public class 학생 {
	...
	// 2. 생성자 [ 객체 선언시 필드의 초기값 ]
		...
		// 2. 생성자 인수는 현재 클래스의 필드명과 동일하게 [ 일반적 ]
			// this.멤버 : 현재 클레스에서의 멤버 접근
	public 학생() { // crtl + space
	}
	...
	// 3. 메소드 =  함수 [  미리 작성된 코드 : 재활용 ]
		...
		// 4. 메소드형태
			// 1. 접근제한자 : 해당 메소드를 호출할 수 있는 범위 제한
				// public : 프로젝트 내 모든곳에서 호출 가능 [ 다른클래스 접근 가능 ]
				// private : 현재 클래스에서만 호출 가능 [ 다른클래스 접근 불가 ]
			// 2. 반환타입[return] : 메소드 종료시 되돌려주는 데이터 [남기는 데이터]
			// 3. 메소드명 : 임의의 이름
			// 4. 메소드명( 인수, 인수2, 인수3 ~~~) : 메소드 안으로 들어오는 데이터
	...
	public void 회원정보() { // 인수 x 반환 x
		// void : return의 데이터가 없다
	...
	public void 나이업데이트(int 나이) { // 인수 o 반환 x
		this.나이 = 나이 ;
	}
}
